...
class LogisticRegression():
    ...
    def update(
        ...
        soft = -np.exp(-y*(x@self.w))/(1+np.exp(-y*(x@self.w)))*y
        
        gradient = (np.sum(soft.reshape(-1,1)*x,axis=0)+self.reg*self.w)
    ...
    def calLossReg(
        ...
        regularization = self.reg*np.linalg.norm(self.w)**2/2
        LossReg = Loss+regularization
        return LossReg
...
x_train, y_train = gen_2D_dataset(100,100,0)
