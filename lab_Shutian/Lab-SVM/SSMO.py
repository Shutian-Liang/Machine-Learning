...
class SSMO_optimizer():
    ...
    def fit(
        ...
        while passes < max_passes:
            num_changed_alphas = 0 # the number of alpha variables that have been changed in this pass, when num_changed_alphas == 0, the training process should be terminated.
            for i in range(N):
                ...
                if self.judge_violoate_KKT(x_train[i],y_train[i],self.alpha[i]).squeeze():
                    other_index = np.delete(index,i)
                    j = np.random.choice(other_index)    
                    ...
                    if eta == 0:
                        ...
                        y_i = y_train[i]
                        y_j = y_train[j]
                        L,H = self.compute_L_H(y_i==y_j,i,j)
                        ...
                        self.alpha[i] = ai_new
                        self.alpha[j] = aj_new
                        self.b = self.compute_new_b(x_train,y_train,i,j,ai_new,aj_new)
                        self.update_SVM()
    ...
    def compute_L_H(
        ...
        # choose bound according to the y1 yj
    ...
    def compute_new_aj(self, x_train, y_train, i, j, eta, L, H):
        ...
        if eta > 0:
            ...
            aj_new_opt = self.alpha[j]+y_train[j]*(E_i-E_j)/eta
            
            # judge whether the a_j_new_opt out of bound
            if aj_new_opt<L:
                aj_new = L
            elif L <= aj_new_opt and aj_new_opt <= H:
                aj_new = aj_new_opt
            else:
                aj_new = H
            return aj_new
